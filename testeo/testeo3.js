...
function palindrome(string) {
    ...
    var sanitized = string.replace(/\W/g, "").toLowerCase(); // todo lo que no sea 
    
    if (sanitized.length ==0)
        return true
    else
        return sanitized == sanitized.split("").reduceRight((sum, v) => sum + v );

}

const palindromo = palindrome("Aaaaaaaab");
