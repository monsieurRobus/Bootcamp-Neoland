export const data = {
    biography: "Muse es una banda de rock británica formada en 1994, en Teignmouth, Devon. Desde su formación, sus integrantes son: Matt Bellamy (voz, guitarra, teclados), Christopher Wolstenholme (bajo, coros) y Dominic Howard (batería).1​2​",
    foundation: 1994,
    genre: ['rock','prog','experimental'],
    logo: 'https://1000marcas.net/wp-content/uploads/2020/03/Logo-Muse-500x313.png',
    albums: [
        {
            title: "Showbiz",
            type: "Album",
            cover: 'https://upload.wikimedia.org/wikipedia/en/9/9d/Museshowbizalbumcover.jpg',
            length: 2976,
            songlist:  [
                {
                    title: 'Sunburn',
                    length: 234
                },
                {
                    title: 'Muscle Museum',
                    length: 263
                },
                {
                    title: 'Fillip',
                    length: 241
                }
                ,{
                    title: 'Falling Down',
                    length: 273
                }
                ,{
                    title: 'Cave',
                    length: 286
                }
                ,{
                    title: 'Showbiz',
                    length: 316
                }
                ,{
                    title: 'Unintended',
                    length: 237
                }
                ,{
                    title: 'Uno',
                    length: 217
                }
                ,{
                    title: 'Sober',
                    length: 244
                }
                ,{
                    title: 'Escape',
                    length: 211
                },
                {
                    title: 'Overdue',
                    length: 146
                },
                {
                    title: 'Hate This and Ill love you',
                    length: 309
                },
                {
                    title: 'Spiral Static (Digital Download edition',
                    length: 284
                }
                
            ]
        },
        {
            title: "Origin of Symmetry",
            type: "Album",
            cover: 'https://upload.wikimedia.org/wikipedia/en/3/35/Muse_-_Origin_of_Symmetry_cover_art.png',
            length: 3101,
            songlist:  [
                {
                    title: 'New Born',
                    length: 363
                },
                {
                    title: 'Bliss',
                    length: 251
                },
                {
                    title: 'Space Dementia',
                    length: 380
                }
                ,{
                    title: 'Hyper Music',
                    length: 201
                }
                ,{
                    title: 'Plug in Baby',
                    length: 219
                }
                ,{
                    title: 'Citizen Erased',
                    length: 441
                }
                ,{
                    title: 'Micro Cuts',
                    length: 218
                }
                ,{
                    title: 'Screenager',
                    length: 260
                }
                ,{
                    title: 'Darkshines',
                    length: 286
                }
                ,{
                    title: 'Feeling Good',
                    length: 198
                },
                {
                    title: 'Megalomania',
                    length: 279
                },
                {
                    title: 'Futurism (Digital Download)',
                    length: 309
                }
                
            ]
        },
        {
            title: "Absolution",
            type: "Album",
            cover: 'https://upload.wikimedia.org/wikipedia/en/b/b4/Muse_-_Absolution_Cover_UK.jpg',
            length: 3139,
            songlist:  [
                {
                    title: 'Intro',
                    length: 22
                },
                {
                    title: 'Apocalypse Please',
                    length: 252
                },
                {
                    title: 'Time is Running Out',
                    length: 236
                }
                ,{
                    title: 'Sing for Absolution',
                    length: 514
                }
                ,{
                    title: 'Stockholm Syndrome',
                    length: 298
                }
                ,{
                    title: 'Interlude',
                    length: 37
                }
                ,{
                    title: 'Hysteria',
                    length: 227
                }
                ,{
                    title: 'Blackout',
                    length: 262
                }
                ,{
                    title: 'Butterflies & Hurricanes',
                    length: 301
                }
                ,{
                    title: 'The Small Print',
                    length: 208
                },
                {
                    title: 'Endlessly',
                    length: 229
                },
                {
                    title: 'Thoughts of a Dying Atheist',
                    length: 191
                },
                {
                    title: 'Rule by Secrecy',
                    length: 294
                },
                {
                    title: 'Fury',
                    length: 302
                }
                
            ]
        },
        {
            title: "Black Holes and Revelations",
            type: "Album",
            cover: 'https://upload.wikimedia.org/wikipedia/en/c/c5/BlackHolesCover.jpg',
            length: 3008,
            songlist:  [
                {
                    title: 'Take a Bow',
                    length: 275
                },
                {
                    title: 'Starlight',
                    length: 240
                },
                {
                    title: 'Supermassive Black Hole',
                    length: 212
                }
                ,{
                    title: 'Map of the Problematique',
                    length: 258
                }
                ,{
                    title: 'Soldier`s Poem',
                    length: 124
                }
                ,{
                    title: 'Invincible',
                    length: 301
                }
                ,{
                    title: 'Assasin',
                    length: 211
                }
                ,{
                    title: 'Exo-Politics',
                    length: 233
                }
                ,{
                    title: 'City of Delusion',
                    length: 288
                }
                ,{
                    title: 'Hoodoo',
                    length: 223
                },
                {
                    title: 'Knights of Cydonia',
                    length: 366
                },
                {
                    title: 'Glorious',
                    length: 281
                }
                
            ]
        }
    ]

}